...
def _device_id_store_path(server_url: str) -> Path:
    """设备ID持久化路径（按server_url隔离）"""
    base = Path.home() / '.py_auth_device'
    base.mkdir(parents=True, exist_ok=True)
    server_hash = hashlib.sha256(server_url.encode('utf-8')).hexdigest()[:12]
    return base / f'device_{server_hash}.txt'


def load_persisted_device_id(server_url: str) -> Optional[str]:
    try:
        path = _device_id_store_path(server_url)
        if path.exists():
...
def persist_device_id(server_url: str, device_id: str) -> None:
    try:
        path = _device_id_store_path(server_url)
...
def build_device_id(server_url: str, provided_device_id: Optional[str], facts: Dict[str, Optional[str]]) -> str:
    if provided_device_id:
        persist_device_id(server_url, provided_device_id)
        return provided_device_id

    if persisted := load_persisted_device_id(server_url):
        return persisted
    ...
    persist_device_id(server_url, device_id)
    return device_id
